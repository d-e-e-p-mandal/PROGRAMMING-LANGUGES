    ...
    sheets.push({
      ...
      frontContent: (
        <div className="h-full w-full p-8 sm:p-10 font-serif text-gray-900 leading-relaxed text-sm sm:text-base overflow-y-auto custom-scrollbar">
           {content1.split('\n').map((p, idx) => <p key={idx} className="mb-4 indent-6 text-justify">{p}</p>)}
           <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-xs text-gray-400 font-sans">{pageIndex1 + 1}</div>
        </div>
      ),
      backContent: (
        <div className="h-full w-full p-8 sm:p-10 font-serif text-gray-900 leading-relaxed text-sm sm:text-base overflow-y-auto custom-scrollbar">
           {content2.split('\n').map((p, idx) => <p key={idx} className="mb-4 indent-6 text-justify">{p}</p>)}
           <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-xs text-gray-400 font-sans">{pageIndex2 + 1}</div>
        </div>
      ),
    });
  }
